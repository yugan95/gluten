    ...
    SQLExecution
      .withThreadLocalCaptured[shard.ShardSetRef](
        ...
        try {
          ...
          val replicaTimeout = Duration(timeout, TimeUnit.MINUTES)
          val numKeyColumns = buildBoundKeys.length
          val blockSize = 4 << 20 // 4MB
          val bloomCapacity = conf.getConf(
            org.apache.spark.sql.internal.SQLConf.DISTRIBUTED_MAP_JOIN_BLOOM_FILTER_CAPACITY)
            ...
            .mapPartitionsWithIndex {
              (shardId, batchIter) =>
                ...
                try {
                  ...
                  // which delegates to GlutenShardLookupService.onInstallReplica
                  // (i.e., GlutenShardManagerJni.constructShardTable), then reports
                  // the shard location to ShardManagerMaster.
                  SparkEnv.get.shardManager.installReplica(setId, shardId)

                  // Request other executors to install replicas (synchronous).
                  SparkEnv.get.shardManager.installReplicaSet(setId, shardId, replicaTimeout)
