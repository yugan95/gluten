...
// ---------------------------------------------------------------------------
// assembleCompactRowWireFromBytes: feed pre-serialized row bytes (taken
// from CompactRow::serialize on individual rows) and verify the assembled
// payload deserializes back to the original RowVector.  This is the path
// used by the stage-2 dedup logic, which serializes each candidate row
// once for both hash-collision tie-break and final wire encoding.
// ---------------------------------------------------------------------------
TEST_F(CompactRowWireFormatTest, AssembleFromBytesRoundTrip) {
  auto input = makeRowVector({
      makeFlatVector<int32_t>({10, 20, 30}),
      makeFlatVector<StringView>({"a"_sv, "bb"_sv, "ccc"_sv}),
  });

  // Serialize each row into its own buffer using CompactRow directly.
  // This mirrors what the dedup pass will do on the probe side.
  facebook::velox::row::CompactRow compactRow(input);
  std::vector<std::string> rowStorage;
  rowStorage.reserve(input->size());
  for (vector_size_t i = 0; i < input->size(); ++i) {
    auto sz = compactRow.rowSize(i);
    std::string buf(sz, '\0');
    auto written = compactRow.serialize(i, buf.data());
    ASSERT_EQ(written, sz);
    rowStorage.emplace_back(std::move(buf));
  }

  std::vector<std::string_view> rowViews;
  rowViews.reserve(rowStorage.size());
  for (const auto& s : rowStorage) {
    rowViews.emplace_back(s.data(), s.size());
  }

  auto wire = assembleCompactRowWireFromBytes(rowViews);

  // Header must be identical to what serializeCompactRowBatch would produce.
  ASSERT_GE(wire.size(), kCompactRowWireHeaderSize);
  uint32_t magic = 0;
  uint32_t version = 0;
  uint32_t numRows = 0;
...
// ---------------------------------------------------------------------------
// assembleCompactRowWireFromBytes: empty input must produce a header-only
// payload (no rows), matching serializeCompactRowBatch(nullptr).
// ---------------------------------------------------------------------------
TEST_F(CompactRowWireFormatTest, AssembleFromBytesEmpty) {
  std::vector<std::string_view> empty;
  auto wire = assembleCompactRowWireFromBytes(empty);
  EXPECT_EQ(wire.size(), kCompactRowWireHeaderSize);
